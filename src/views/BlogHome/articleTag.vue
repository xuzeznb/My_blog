<template>
  <div  v-lazy:backhround-image="home_info.home_background"
    ...
    <!-- 标签分类   -->
    <div class="active_top" style="position: relative; top: 5px; height: 420px; overflow: auto" v-show="route_content">
      <div>
        <h3><a href="javascript:;" style="text-decoration: none; padding: 0  15px"><el-icon style="padding: 0 5px" size="15"><Back /></el-icon> 返回</a>当前<strong style="padding:  0  10px">{{ route_content  }}</strong>标签下的文章</h3>
      </div>
      <el-divider></el-divider>
      <div style="display:flex;justify-content:left ">
        <div v-for="item in article_id" style="padding: 10px" @click="router_path(item.article_id)">
        <div class="tag_content" style=" width: 260px;background:#f7f4f4;border-radius:10px;height: auto">
          <a href="javascript:;" style=" display: flex;padding:10px 5px 10px 20px;" v-html="item.article_title"></a>
          ...
          <p style=" padding:10px 20px;font-size: 10px" v-html="utils.formatDate(item.creat_time)"></p>
        </div>
      </div>
      </div>
    </div>
